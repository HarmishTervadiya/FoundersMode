...
interface AuthState {
  ...
  signInWithKey: (secretKey: string) => Promise<{ data: any; error: string | null }>;
      ...
      supabase.auth.onAuthStateChange(async (_event, session) => {
        ...
        if (_event === 'SIGNED_IN' && session?.user) {
          ...
          try {
            ...
            if (!current) {
              await AsyncStorage.setItem(LOCAL_LAST_LOGIN_KEY, new Date().toISOString());
            }
          } catch (e) {
        ...
        if (result.type === 'success' && result.url) {
          ...
          if (access_token && refresh_token) {
            ...
            if (sessionData?.session) {
              ...
              await AsyncStorage.setItem(LOCAL_LAST_LOGIN_KEY, new Date().toISOString());
              console.log('[AuthStore] Session stored! User ID:', sessionData.session.user?.id);
            }
          } else {
            console.log('[AuthStore] No tokens found in URL, trying getSession...');
            ...
            if (session) {
              ...
              await AsyncStorage.setItem(LOCAL_LAST_LOGIN_KEY, new Date().toISOString());
              console.log('[AuthStore] Fallback session found:', session.user?.id);
            }
          }
