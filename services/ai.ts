...
export const ai = {
  ...
  analyzeLog: async (logContent: string): Promise<LogAnalysisResult> => {
    ...
    if (!API_KEY) {
      console.error(
        "Gemini API Key is missing. Ensure EXPO_PUBLIC_GEMINI_API_KEY is set."
      );
      throw new Error(
        "System Error: Neural Link Disconnected (Missing API Key)."
      );
    }

    try {
      const prompt = GAME_MASTER_SYSTEM_PROMPT.replace(
        "{user_log_text}",
        logContent
      );
      ...
      if (!response || !response.text) {
        throw new Error(
          "Analysis failed: received empty data from the Game Engine."
        );
      }
      ...
      try {
        ...
        throw new Error(
          "Data Corruption: Unable to decode Game Engine response."
        );
      }
    } catch (error: any) {
